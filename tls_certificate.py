...
def carregar_ou_criar_ca(agora, senha):
    if CA_KEY.exists() and CA_CERT.exists():
        try:
            ...
            if chave.public_key().public_numbers() != certificado.public_key().public_numbers():
                raise ValueError("A chave privada da CA nao pertence ao certificado da CA.")
            if certificado.subject != certificado.issuer:
                raise ValueError("O certificado da CA local nao e autoassinado.")
            ...
            restricoes = certificado.extensions.get_extension_for_class(
                x509.BasicConstraints
            ).value
            if not restricoes.ca:
                raise ValueError("O certificado carregado nao e uma CA.")
            certificado.extensions.get_extension_for_class(x509.SubjectKeyIdentifier)
    ...
    nome_host = socket.gethostname().strip().lower() or "servidor"
    nome = x509.Name([
        x509.NameAttribute(
            NameOID.COMMON_NAME,
            f"Dashboard Monitor Local CA - {nome_host}",
        )
    ])
    certificado = (
