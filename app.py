...
from database import (
    ...
    coluna_existe,
    criar_schema,
    garantir_tabela_configuracoes_agentes,
    get_db,
)
...
CONFIGURACOES_MASTER_PATH = os.getenv(
    "DASHBOARD_AGENT_CONFIG_MASTER_FILE",
    os.path.join(BASE_DIR, ".agent_config_master.dpapi"),
)
CONFIGURACOES_MASTER_SECRET = carregar_ou_criar_segredo(
    CONFIGURACOES_MASTER_PATH,
    64,
    "Chave mestra das configuracoes dos agentes",
)


def _chave_fernet_dominio(dominio):
    digest = hmac.new(
        CONFIGURACOES_MASTER_SECRET,
        str(dominio).encode("utf-8"),
        hashlib.sha256,
    ).digest()
    return base64.urlsafe_b64encode(digest)


def chave_pareamento_loja(loja_id):
    return _chave_fernet_dominio(f"agente-loja:{int(loja_id)}").decode("ascii")


def cifrar_configuracao_banco(loja_id, configuracao):
    conteudo = {
        "versao": 1,
        "loja_id": int(loja_id),
        "configuracao": configuracao,
    }
    dados = json.dumps(conteudo, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    return Fernet(_chave_fernet_dominio("mysql-configuracoes-agentes")).encrypt(dados)


def decifrar_configuracao_banco(loja_id, blob):
    try:
        dados = Fernet(_chave_fernet_dominio("mysql-configuracoes-agentes")).decrypt(bytes(blob))
        conteudo = json.loads(dados.decode("utf-8"))
    except (InvalidToken, UnicodeError, ValueError, TypeError) as exc:
        raise RuntimeError("Configuracao cifrada da loja esta corrompida ou pertence a outra chave.") from exc
    if conteudo.get("versao") != 1 or int(conteudo.get("loja_id", 0)) != int(loja_id):
        raise RuntimeError("Configuracao cifrada nao pertence a esta loja.")
    configuracao = conteudo.get("configuracao")
    if not isinstance(configuracao, dict):
        raise RuntimeError("Conteudo da configuracao cifrada e invalido.")
    return configuracao
...
AGENTE_CAIXAS_TIMEOUT = float(os.getenv("AGENTE_CAIXAS_TIMEOUT", "25"))
...
def _config_texto(config, chave, limite, obrigatorio=True, padrao=""):
    valor = str(config.get(chave, padrao) or "").strip()
    if (obrigatorio and not valor) or len(valor) > limite:
        raise ValueError(f"campo_config_invalido:{chave}")
    if any(ord(char) < 32 for char in valor):
        raise ValueError(f"campo_config_invalido:{chave}")
    return valor


def _config_inteiro(config, chave, minimo, maximo, padrao):
    try:
        valor = int(config.get(chave, padrao))
    except (TypeError, ValueError) as exc:
        raise ValueError(f"campo_config_invalido:{chave}") from exc
    if not minimo <= valor <= maximo:
        raise ValueError(f"campo_config_invalido:{chave}")
    return valor


def configuracao_agente_padrao():
    return {
        "banco_syspdv": {
            "servidor": "127.0.0.1",
            "porta": 1433,
        ...
        "firebird_caixas": {
            "porta": 3050,
            "banco": "/opt/Syspdv/syspdv_mov.fdb",
            "usuario": "SYSDBA",
            "senha": "",
            "charset": "WIN1252",
            "timeout_segundos": 3,
...
def validar_configuracao_agente(dados, atual=None):
    if not isinstance(dados, dict):
        raise ValueError("configuracao_invalida")
    banco = dados.get("banco_syspdv")
    firebird = dados.get("firebird_caixas")
    if not isinstance(banco, dict) or not isinstance(firebird, dict):
        raise ValueError("configuracao_invalida")
    atual = atual or configuracao_agente_padrao()

    autenticacao_windows = banco.get("autenticacao_windows", False)
    if not isinstance(autenticacao_windows, bool):
        raise ValueError("campo_config_invalido:autenticacao_windows")
    senha_sql = str(banco.get("senha", ""))
    if not senha_sql:
        senha_sql = str(atual.get("banco_syspdv", {}).get("senha", ""))
    usuario_sql = _config_texto(banco, "usuario", 128, not autenticacao_windows)
    if not autenticacao_windows and not senha_sql:
        raise ValueError("senha_sql_obrigatoria")
    if len(senha_sql) > 512 or any(ord(char) < 32 for char in senha_sql):
        raise ValueError("senha_sql_invalida")

    senha_firebird = str(firebird.get("senha", ""))
    if not senha_firebird:
        senha_firebird = str(atual.get("firebird_caixas", {}).get("senha", ""))
    if not senha_firebird or len(senha_firebird) > 512 or any(ord(char) < 32 for char in senha_firebird):
        raise ValueError("senha_firebird_obrigatoria")

    return {
        "banco_syspdv": {
            "servidor": _config_texto(banco, "servidor", 255, True, "127.0.0.1"),
            "porta": _config_inteiro(banco, "porta", 1, 65535, 1433),
            "banco": _config_texto(banco, "banco", 128),
            "usuario": usuario_sql,
            "senha": senha_sql,
            "autenticacao_windows": autenticacao_windows,
            "driver": _config_texto(banco, "driver", 100, True, "auto"),
            "timeout_segundos": _config_inteiro(banco, "timeout_segundos", 2, 60, 15),
            "limite_registros": _config_inteiro(banco, "limite_registros", 1, 10000, 2000),
        },
        "firebird_caixas": {
            "porta": _config_inteiro(firebird, "porta", 1, 65535, 3050),
            "banco": _config_texto(
                firebird, "banco", 500, True, "/opt/Syspdv/syspdv_mov.fdb"
            ),
            "usuario": _config_texto(firebird, "usuario", 128, True, "SYSDBA"),
            "senha": senha_firebird,
            "charset": _config_texto(firebird, "charset", 40, True, "WIN1252"),
            "timeout_segundos": _config_inteiro(firebird, "timeout_segundos", 1, 15, 3),
        },
    }


def obter_configuracao_agente(conn, loja_id):
    registro = conn.execute(
        "SELECT * FROM configuracoes_agentes WHERE loja_id = ?", (loja_id,)
    ).fetchone()
    if not registro:
        return None, None
    return registro, decifrar_configuracao_banco(loja_id, registro["configuracao_cifrada"])


def configuracao_agente_para_json(configuracao):
    config = json.loads(json.dumps(configuracao))
    senha_sql = bool(config["banco_syspdv"].get("senha"))
    senha_firebird = bool(config["firebird_caixas"].get("senha"))
    config["banco_syspdv"]["senha"] = ""
    config["firebird_caixas"]["senha"] = ""
    return config, {
        "sql_server": senha_sql,
        "firebird": senha_firebird,
    }
...
def init_db():
    ...
    if auto_migrar:
        ...
        # Esta tabela faz parte da funcionalidade normal do painel e precisa
        # existir mesmo quando as migracoes gerais do MySQL estao desativadas.
        garantir_tabela_configuracoes_agentes(conn)
...
def autorizar_acesso_por_permissao():
    ...
    rota_configuracoes_agentes = (
        request.path == "/cadastro/configuracoes-agentes"
        or request.path.startswith("/api/cadastro/configuracoes-agentes")
    )
    if rota_configuracoes_agentes:
        permitido = bool(registro["administrador"])
    elif request.path == "/cadastro/usuarios" or request.path.startswith("/api/usuarios"):
        permitido = bool(registro["administrador"])
    else:
        regras = (
...
def sincronizar_configuracao_com_agente(loja, configuracao, alterado_em):
    ip_loja = validar_ip_privado(loja["ip"])
    payload = {
        "versao": 1,
        "loja_id": int(loja["id"]),
        "alterado_em": alterado_em,
        "banco_syspdv": configuracao["banco_syspdv"],
        "firebird_caixas": configuracao["firebird_caixas"],
    }
    token = Fernet(chave_pareamento_loja(loja["id"]).encode("ascii")).encrypt(
        json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    ).decode("ascii")
    corpo = json.dumps({"token": token}, separators=(",", ":")).encode("utf-8")
    conexao = http.client.HTTPConnection(ip_loja, AGENTE_PORTA, timeout=max(5, AGENTE_TIMEOUT))
    try:
        conexao.request(
            "POST",
            "/configuracao",
        ...
        conteudo = resposta.read(64 * 1024 + 1)
        if len(conteudo) > 64 * 1024:
            raise RuntimeError("Resposta do agente excedeu o limite permitido.")
        try:
            dados = json.loads(conteudo.decode("utf-8")) if conteudo else {}
        except (UnicodeError, ValueError):
            dados = {}
        if resposta.status != 200:
            mensagem = str(dados.get("mensagem") or dados.get("erro") or "Agente recusou a configuracao.")
            raise RuntimeError(mensagem[:300])
        return dados
    except (OSError, http.client.HTTPException) as exc:
        raise RuntimeError("Nao foi possivel conectar ao agente da loja.") from exc
    finally:
        conexao.close()


def testar_configuracao_com_agente(loja, configuracao):
    ip_loja = validar_ip_privado(loja["ip"])
    payload = {
        "versao": 1,
        "loja_id": int(loja["id"]),
    ...
    timeout = min(
        ...
        max(
            15,
            int(configuracao["banco_syspdv"]["timeout_segundos"])
            + int(configuracao["firebird_caixas"]["timeout_segundos"])
            + 10,
        ),
    )
    conexao = http.client.HTTPConnection(ip_loja, AGENTE_PORTA, timeout=timeout)
    try:
        conexao.request(
            "POST",
            "/configuracao/testar",
        ...
        if resposta.status != 200:
            mensagem = str(
                dados.get("mensagem") or dados.get("erro") or "Agente recusou o teste."
            )
            raise RuntimeError(mensagem[:300])
        return dados
    except (OSError, http.client.HTTPException) as exc:
...
def caixas_descobertos_firebird(caixas_agente, nome_loja="", caixas_rows=None):
    """Monta a lista pelo par CXANUM/IP e complementa observacoes do cadastro."""
    cadastro_por_ip = {}
    cadastro_por_numero = {}
    for row in caixas_rows or []:
        ip_cadastro = normalizar(row["ip"])
        numero_cadastro = numero_loja(row["nome"])
        if ip_cadastro:
            cadastro_por_ip[ip_cadastro] = row
        if numero_cadastro != 9999:
            cadastro_por_numero[numero_cadastro] = row

    caixas = []
    vistos = set()
    for item in caixas_agente or []:
        if not isinstance(item, dict):
            continue
        try:
            numero_caixa = int(item.get("numero_caixa", item.get("cxanum")))
        except (TypeError, ValueError):
            continue
        ...
        chave = (normalizar(ip), numero_caixa)
        if chave in vistos:
            continue
        vistos.add(chave)

        cadastro = cadastro_por_ip.get(normalizar(ip))
        if cadastro is None:
            cadastro = cadastro_por_numero.get(numero_caixa)

        status = normalizar(item.get("status", "offline"))
        nome = f"Caixa {numero_caixa}"
        caixas.append({
            "id": cadastro["id"] if cadastro is not None else None,
            "loja": nome_loja,
            "caixa": nome,
            "nome": nome,
            ...
            "ip": ip,
            "observacao": (
                str(cadastro["observacao"] or "").strip()
                if cadastro is not None
                else str(item.get("observacao") or "").strip()
            ),
            "status": "online" if status == "online" else "offline",
            "syspdv_caixa": "offline",
            "fonte_status": "sqlserver_firebird",
        })
    return caixas


def caixas_informados_agente(caixas_agente, nome_loja=""):
    """Normaliza caixas recebidos do agente sem consultar o cadastro do MySQL."""
    caixas = []
    vistos = set()
    ...
    for item in caixas_agente or []:
        ...
        ip = str(item.get("ip") or "").strip()
        numero_caixa = item.get("numero_caixa", item.get("cxanum"))
        try:
            numero_caixa = int(numero_caixa) if numero_caixa is not None else None
        except (TypeError, ValueError):
            numero_caixa = None

        chave = (normalizar(ip), normalizar(nome), numero_caixa)
        if chave in vistos:
            continue
        vistos.add(chave)
        ...
        status = normalizar(item.get("status", "offline"))
        syspdv_caixa = normalizar(
            item.get("syspdv_caixa")
            or item.get("syspdv")
            or item.get("status_syspdv")
            or item.get("Syspdv_pdv.exe")
            or "offline"
        )
        nome_exibicao = nome or (
            f"Caixa {numero_caixa}" if numero_caixa is not None else ip or "Caixa"
        )
        ...
        caixas.append({
            ...
            "syspdv_caixa": (
                syspdv_caixa
                if syspdv_caixa in ["online", "offline", "erro"]
                else "offline"
            ),
            "fonte_status": "agente",
        })
...
def carregar_dados_caixas_loja(nome_loja):
    """
    Exibe exclusivamente os caixas informados pelo agente da loja.
    O cadastro de caixas do MySQL nao e usado como contingencia.
    """
    conn = get_db()
    cur = conn.cursor()
    ...
    agente_data = consultar_agente_loja(
        loja_info["ip"], endpoint="/caixas", timeout=AGENTE_CAIXAS_TIMEOUT
    )
    ...
    fonte_firebird = agente_online and agente_data.get("fonte_caixas") == "sqlserver_firebird"
    if fonte_firebird:
        caixas = caixas_descobertos_firebird(
            caixas_agente,
            nome_loja=loja_info["nome"],
            caixas_rows=caixas_rows,
        )
    else:
        caixas = caixas_informados_agente(caixas_agente, nome_loja=loja_info["nome"])
    ...
    resposta = {
        ...
        "fonte": "sqlserver_firebird" if fonte_firebird else (
            "agente" if agente_online else "agente_indisponivel"
        )
    }
    ...
    if agente_online:
        resposta.update({
            ...
            "agente_ips_descobertos": agente_data.get("ips_descobertos", 0),
            "agente_ips_consultados": agente_data.get("ips_consultados", 0),
            "agente_caixas_ativos_sql": agente_data.get("caixas_ativos_sql", len(caixas)),
            "agente_conexoes_nao_associadas": agente_data.get("conexoes_nao_associadas", []),
            "agente_falhas_firebird": agente_data.get("falhas_firebird", [])
        })
    else:
...
def pagina_usuarios():
    ...
    try:
        registros = conn.execute(
            "SELECT * FROM usuarios ORDER BY administrador DESC, LOWER(nome), nome"
        ).fetchall()
    finally:
        conn.close()
    return render_template(
...
@app.route("/cadastro/configuracoes-agentes")
@login_required
def pagina_configuracoes_agentes():
    conn = get_db()
    try:
        lojas = conn.execute("SELECT id, nome, ip FROM lojas ORDER BY nome").fetchall()
    finally:
        conn.close()
    return render_template(
        "configuracoes_agentes.html",
        usuario=session["user"],
        lojas=[dict(item) for item in lojas],
    )


def _buscar_loja_configuracao(conn, loja_id):
    loja = conn.execute("SELECT id, nome, ip FROM lojas WHERE id = ?", (loja_id,)).fetchone()
    if not loja:
        raise LookupError("loja_nao_encontrada")
    return loja


def _registrar_resultado_sincronizacao(loja_id, sucesso, erro=""):
    conn = get_db()
    try:
        if sucesso:
            conn.execute(
                """
                UPDATE configuracoes_agentes
                SET sincronizado_em = ?, ultimo_erro = NULL
                WHERE loja_id = ?
                """,
                (now_str(), loja_id),
            )
...
@app.route("/api/cadastro/configuracoes-agentes/<int:loja_id>", methods=["GET", "PUT"])
@login_required
def api_configuracao_agente(loja_id):
    conn = get_db()
    try:
        loja = _buscar_loja_configuracao(conn, loja_id)
        registro, atual = obter_configuracao_agente(conn, loja_id)
        if request.method == "GET":
            configuracao, senhas = configuracao_agente_para_json(
                atual or configuracao_agente_padrao()
            )
            return jsonify({
                "loja": dict(loja),
                "configurado": bool(registro),
                "configuracao": configuracao,
                "senhas_configuradas": senhas,
                "versao": int(registro["versao"]) if registro else 0,
                "alterado_por": registro["alterado_por"] if registro else "",
                "alterado_em": registro["alterado_em"] if registro else "",
                "sincronizado_em": registro["sincronizado_em"] if registro else "",
                "ultimo_erro": registro["ultimo_erro"] if registro else "",
            })

        dados = request.get_json(silent=True) or {}
        configuracao = validar_configuracao_agente(dados.get("configuracao"), atual=atual)
        alterado_em = now_str()
        versao = int(registro["versao"] or 0) + 1 if registro else 1
        blob = cifrar_configuracao_banco(loja_id, configuracao)
        parametros = (
            loja_id, blob, versao, session.get("user", ""), alterado_em,
        )
        if DB_BACKEND == "sqlite":
            conn.execute("""
                INSERT INTO configuracoes_agentes
                    (loja_id, configuracao_cifrada, versao, alterado_por, alterado_em,
                     sincronizado_em, ultimo_erro)
                VALUES (?, ?, ?, ?, ?, NULL, NULL)
                ON CONFLICT(loja_id) DO UPDATE SET
                    configuracao_cifrada = excluded.configuracao_cifrada,
            ...
            conn.execute("""
                ...
                ON DUPLICATE KEY UPDATE
                    configuracao_cifrada = VALUES(configuracao_cifrada),
        ...
        return jsonify({
            "erro": str(erro).split(":", 1)[0],
            "mensagem": "Revise os campos obrigatorios e os limites informados.",
        }), 400
    except Exception as erro:
        logger.exception("Falha ao acessar configuracao protegida da loja %s: %s", loja_id, erro)
        return jsonify({
            "erro": "configuracoes_indisponiveis",
            "mensagem": "Aplique a migracao MySQL das configuracoes dos agentes.",
        }), 503
    finally:
        conn.close()
    ...
    erro_sincronizacao = ""
    try:
        sincronizar_configuracao_com_agente(loja_dict, configuracao, alterado_em)
        sincronizado = True
    except RuntimeError as erro:
        erro_sincronizacao = str(erro)
    _registrar_resultado_sincronizacao(loja_id, sincronizado, erro_sincronizacao)
    logger.info(
        "Configuracao do agente da loja %s alterada por %s; sincronizada=%s.",
        loja_id, session.get("user", ""), sincronizado,
    )
    return jsonify({
        "ok": True,
        "mensagem": (
            "Configuracao salva e sincronizada com o agente."
            if sincronizado else
            "Configuracao salva no MySQL; o agente ainda nao recebeu a atualizacao."
        ),
...
@app.route("/api/cadastro/configuracoes-agentes/<int:loja_id>/sincronizar", methods=["POST"])
@login_required
def api_sincronizar_configuracao_agente(loja_id):
    conn = get_db()
    try:
        loja = _buscar_loja_configuracao(conn, loja_id)
        registro, configuracao = obter_configuracao_agente(conn, loja_id)
        if not registro or not configuracao:
            return jsonify({"erro": "configuracao_nao_encontrada"}), 404
        loja_dict = dict(loja)
        alterado_em = registro["alterado_em"]
    except LookupError:
        return jsonify({"erro": "loja_nao_encontrada"}), 404
    except Exception as erro:
        logger.exception("Falha ao preparar sincronizacao da loja %s: %s", loja_id, erro)
        return jsonify({
            "erro": "configuracoes_indisponiveis",
            "mensagem": "Aplique a migracao MySQL das configuracoes dos agentes.",
        }), 503
    ...
    try:
        sincronizar_configuracao_com_agente(loja_dict, configuracao, alterado_em)
        _registrar_resultado_sincronizacao(loja_id, True)
        return jsonify({"ok": True, "mensagem": "Agente sincronizado com sucesso."})
    except RuntimeError as erro:
        mensagem = str(erro)
        _registrar_resultado_sincronizacao(loja_id, False, mensagem)
        return jsonify({"erro": "falha_sincronizacao", "mensagem": mensagem}), 502


@app.route("/api/cadastro/configuracoes-agentes/<int:loja_id>/testar", methods=["POST"])
@login_required
def api_testar_configuracao_agente(loja_id):
    conn = get_db()
    try:
        loja = _buscar_loja_configuracao(conn, loja_id)
        _, atual = obter_configuracao_agente(conn, loja_id)
        dados = request.get_json(silent=True) or {}
        configuracao = validar_configuracao_agente(dados.get("configuracao"), atual=atual)
        loja_dict = dict(loja)
    except LookupError:
        ...
        return jsonify({
            "erro": str(erro).split(":", 1)[0],
            "mensagem": "Revise os campos e informe as senhas que ainda nao foram salvas.",
        }), 400
    except Exception as erro:
        logger.exception("Falha ao preparar teste da loja %s: %s", loja_id, erro)
        return jsonify({"erro": "teste_indisponivel", "mensagem": str(erro)[:300]}), 503
    finally:
        conn.close()

    try:
        resultado = testar_configuracao_com_agente(loja_dict, configuracao)
        logger.info("Conexoes do agente da loja %s testadas por %s.", loja_id, session.get("user", ""))
        return jsonify(resultado)
    except RuntimeError as erro:
        return jsonify({"erro": "falha_teste", "mensagem": str(erro)}), 502


@app.route("/api/cadastro/configuracoes-agentes/<int:loja_id>/pareamento")
@login_required
def api_pareamento_configuracao_agente(loja_id):
    conn = get_db()
    try:
        loja = _buscar_loja_configuracao(conn, loja_id)
    except LookupError:
    ...
    return jsonify({
        "loja": dict(loja),
        "chave": chave_pareamento_loja(loja_id),
        "instrucao": (
            "No servidor da loja, execute configurar_pareamento.ps1 como Administrador "
            "e cole esta chave quando solicitado."
        ),
    })
...
def api_caixas_debug():
    ...
    agente_caixas = consultar_agente_loja(
        loja["ip"], endpoint="/caixas", timeout=AGENTE_CAIXAS_TIMEOUT
    )

    return jsonify({
        "loja_banco": dict(loja),
        "caixas_banco": caixas_banco,
        "agente_timeout": AGENTE_CAIXAS_TIMEOUT,
